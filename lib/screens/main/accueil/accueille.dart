...
// Les couleurs sont maintenant gérées par ThemeService
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  List<Map<String, dynamic>> get _defaultRecentActivities => [
    {'icon': Icons.check_circle, 'color': widget.themeService.activitySuccess, 'text': 'Quiz sur le livre Harry Potter à l\'école, il y a 2 jours'},
    {'icon': Icons.book, 'color': widget.themeService.activityBook, 'text': 'Nouveau livre débuté, il y a 4 jours'},
    {'icon': Icons.emoji_events, 'color': widget.themeService.activityTrophy, 'text': 'Dernière challenge participé, il y a 2 jours'},
  ];
  ...
  List<BadgeEleveResponse>? _allBadgesWithStatus; // Tous les badges avec statut d'obtention
                        ...
                        child: Column(
                          ...
                          children: [
                            Text(
                              'Bienvenue',
                              style: TextStyle(
                                color: widget.themeService.colorWhite,
                                fontSize: 14,
                        ...
                        children: [
                          GestureDetector(
                            ...
                            child: Container(
                              ...
                              child: Row(
                                ...
                                children: [
                                  Icon(Icons.star, color: widget.themeService.badgeGold, size: 18),
                          ...
                          GestureDetector(
                            ...
                            child: NotificationBadge(
                              ...
                              child: Icon(
                                Icons.notifications,
                                color: widget.themeService.badgeGold,
                                size: 26,
                              ),
                            ),
                          ),
  ...
  Widget _buildUserAvatar(Color primaryColor) {
    ...
    return CircleAvatar(
      radius: 30,
      backgroundColor: widget.themeService.colorWhite,
          ...
          children: [
            Row(
              ...
              children: [
                Row(
                  children: [
                    Text('Défi du jour', style: TextStyle(color: widget.themeService.colorBlack, fontSize: 16, fontWeight: FontWeight.bold)),
                    const SizedBox(width: 5),
            ...
            ClipRRect(
              ...
              child: LinearProgressIndicator(
                ...
                valueColor: AlwaysStoppedAnimation<Color>(
                    _isChallengeCompleted ? primaryColor : widget.themeService.challengeWarning
                ),
                minHeight: 8,
              ),
            ),
        ...
        Row(
          ...
          children: [
            Text(
              ...
              style: TextStyle(
                color: widget.themeService.colorBlack,
                fontSize: 20,
                fontWeight: FontWeight.bold,
              ),
            ),
    ...
    return Column(
      ...
      children: [
        Row(
          mainAxisAlignment: MainAxisAlignment.spaceBetween,
          children: [
            Text('Succès et Badges', style: TextStyle(color: widget.themeService.colorBlack, fontSize: 20, fontWeight: FontWeight.bold)),
            TextButton(
              onPressed: () {
                Navigator.push(
                  context,
    ...
    return Column(
      ...
      children: [
        Row(
          mainAxisAlignment: MainAxisAlignment.spaceBetween,
          children: [
            Text('Activité Récentes', style: TextStyle(color: widget.themeService.colorBlack, fontSize: 20, fontWeight: FontWeight.bold)),
            TextButton(
              onPressed: () {
                Navigator.push(
                  context,
        ...
        Container(
          ...
          child: Column(
            children: activities.map((activity) =>
                _ActivityRow(icon: activity['icon'], color: activity['color'], text: activity['text'], themeService: widget.themeService)
            ).toList(),
          ),
        ),
      ],
  ...
  List<Widget> _buildBadgeItems(Color primaryColor) {
    // Si on a tous les badges avec statut, filtrer pour n'afficher que les badges obtenus
    if (_allBadgesWithStatus != null && _allBadgesWithStatus!.isNotEmpty) {
      // Filtrer uniquement les badges obtenus
      final obtainedBadges = _allBadgesWithStatus!
          .where((badge) => badge.obtenu == true)
          .take(3)
          .toList();
      
      if (obtainedBadges.isEmpty) {
        // Si aucun badge obtenu, afficher un message ou retourner une liste vide
        return [
          Padding(
            ...
            child: Text(
              ...
              style: TextStyle(
                color: widget.themeService.colorBlack.withOpacity(0.6),
                fontSize: 14,
      ...
      return obtainedBadges.map((BadgeEleveResponse badge) {
        Color color;
        ...
        switch (badge.type?.toUpperCase() ?? '') {
          case 'OR':
            ...
            break;
          case 'ARGENT':
            color = widget.themeService.badgeSilver;
            break;
          case 'BRONZE':
            color = widget.themeService.badgeBronze;
            break;
          case 'PROGRESSION':
            color = widget.themeService.badgeProgression; // Utiliser la couleur du thème
            break;
          case 'SPECIAL':
          case 'CHALLENGE':
          case 'CLASSEMENT':
        ...
        return _BadgeItem(
          ...
          isUnlocked: true, // Tous les badges affichés sont obtenus
          themeService: widget.themeService,
        );
      }).toList();
    }
    
    // Fallback: Si on a tous les badges disponibles, filtrer pour n'afficher que ceux obtenus
    if (_allBadges != null && _allBadges!.isNotEmpty && _badges != null && _badges!.isNotEmpty) {
      final earnedBadgeIds = _badges!.map((b) => b.id).toSet();
      
      // Filtrer uniquement les badges obtenus
      final obtainedBadges = _allBadges!
          .where((badge) => badge.id != null && earnedBadgeIds.contains(badge.id))
          .take(3)
          .toList();
      
      if (obtainedBadges.isEmpty) {
        // Si aucun badge obtenu, afficher un message
        return [
          Padding(
            padding: const EdgeInsets.all(16.0),
            child: Text(
      ...
      return obtainedBadges.map((BadgeModel.Badge badge) {
        Color color;
        switch (badge.type) {
          ...
          case BadgeModel.BadgeTypeEnum.BRONZE:
            color = widget.themeService.badgeBronze;
            break;
          case BadgeModel.BadgeTypeEnum.PROGRESSION:
            color = widget.themeService.badgeProgression; // Utiliser la couleur du thème
            break;
          default:
            color = primaryColor;
    ...
    // Dernier fallback: Afficher uniquement les badges obtenus (sans badges verrouillés par défaut)
    if (_badges != null && _badges!.isNotEmpty) {
      return _badges!.take(3).map((BadgeModel.Badge badge) {
        Color color;
        switch (badge.type) {
          case BadgeModel.BadgeTypeEnum.OR:
            color = widget.themeService.badgeGold;
              ...
              children: [
                ElevatedButton(
                  ...
                  style: ElevatedButton.styleFrom(
                    backgroundColor: widget.themeService.colorWhite,
                    foregroundColor: widget.themeService.colorBlack,
                ...
                ElevatedButton(
                  ...
                  style: ElevatedButton.styleFrom(
                    ...
                    foregroundColor: widget.themeService.colorWhite,
                    padding: const EdgeInsets.symmetric(horizontal: 30, vertical: 15),
                    shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(30)),
                    elevation: 2,
                  ),
                  child: _isLoading
                      ? SizedBox(
                          ...
                          child: CircularProgressIndicator(
                            strokeWidth: 2,
                            valueColor: AlwaysStoppedAnimation<Color>(widget.themeService.colorWhite),
                          ),
                        )
      ...
      content: Column(
        ...
        children: [
          ...
          ElevatedButton(
            ...
            style: ElevatedButton.styleFrom(
              backgroundColor: primaryColor,
              foregroundColor: themeService.colorWhite,
              padding: const EdgeInsets.symmetric(horizontal: 40, vertical: 15),
              shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(30)),
            ),
      ...
      content: Column(
        ...
        children: [
          ...
          Container(
            padding: const EdgeInsets.all(10),
            decoration: BoxDecoration(
              color: themeService.readingProgressComplete.withOpacity(0.1),
              borderRadius: BorderRadius.circular(5),
            ),
            child: Text(
              'Bonne réponse\nH2O',
              style: TextStyle(
                color: themeService.readingProgressComplete,
                fontSize: 14,
              ),
            ),
          ),
...
class _BadgeItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final textColor = isUnlocked ? themeService.colorBlack : Colors.grey.shade400;
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          const SizedBox(width: 15),
          Expanded(child: Text(text, style: TextStyle(color: themeService.colorBlack, fontSize: 15))),
        ],
      ),
    );
  }
...
class _ReadingProgressRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    Color progressColor = isFinished 
        ? themeService.readingProgressComplete
        : (reading.progress < 0.40 ? themeService.readingProgressLow : primaryColor);
                  ...
                  child: Row(
                    children: [
                      ...
                      if (isFinished) ...[
                        ...
                        Icon(Icons.check_circle, color: themeService.readingProgressComplete, size: 18),
                      ],
                    ],
                  ),
                ),
